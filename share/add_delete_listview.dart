...
class AddDeleteListView<T> extends StatefulWidget {
  ...
  final ItemAction<T> onAdd, onTap;
  final ItemAction<T>? onDelete, onPressEdit;
...
class _AddDeleteListViewState<T> extends State<AddDeleteListView<T>> {
  ...
  Widget _tile(T item, BuildContext context) {
    var listTile = ListTile(
      leading: widget.leadingIcon,
      title: Row(children: [
        Expanded(child: widget.titleBuilder(context, item)),
        if (widget.onPressEdit != null)
          IconButton(
            ...
            onPressed: () async => widget.onPressEdit!(item),
          ),
        if (widget.onDelete != null)
          IconButton(
              ...
              onPressed: () async {
                ...
                setState(() => widget.backingList.remove(item));
              })
      ]),
      onTap: () async => await widget.onTap(item),
    );
    return listTile;
  }

  Widget _adderForm() {
    var backingList = widget.backingList;
    return Form(
        key: _formKey,
        child: Padding(
  ...
  Widget build(BuildContext context) {
    return Center(
      ...
      children: [
        _adderForm(),
        for (var i in widget.backingList.reversed) _tile(i, context),
      ],
    ));
  }
}
