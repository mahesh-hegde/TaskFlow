...
class _ScheduleViewState extends State<ScheduleView> {
  ...
  void _onTick(Timer timer) {
    ...
    if (_minutes >= assignedMinutes) {
      ...
      // TODO: Show ComponentTimeOverNotification
      // Providing options to snooze (+5) or NEXT
      if (_index == comps.length) {
        _index--; // pause at last stage
        _timer?.cancel();
        _timer = null;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    void gotoNext() {
      setState(() {
        _resetTicks();
        if (_index == comps.length - 1) {
          _markAllDone();
        }
        _index = min(_index + 1, comps.length - 1);
      });
    }
            ...
            steps: [
              for (var comp in comps)
                ...
                Step(
                    ...
                    // TODO: Add buttons to edit time temporarily, tune out, postpone to end
                    ),
            ],
          );
    return Scaffold(
...
class _ComponentTitleState extends State<ComponentTitle> {
  ...
  void _showJournalEditor() async {
    ...
    // delete & return if entry was cleared.
    ...
    setState(() {});
    await DataStore.database.update('journal_entry', {'entry': record.entry},
