...
class MyApp extends StatelessWidget {
 TodoListBaseodoListBaseodoListBaseconst MyApp({Key? key}) : super(key: key);
...
class AllTodoLists extends StatelessWidget {
 ...
 TodoTaskBaseodoTaskBase    titleBuilder: (context, todolist) => Text(todolist.name),
  ...
  Widget build(BuildContext context) {
    return AddDeleteListView<Schedule>(
      ...
      onAdd: (newSched) async {
        newSched.id =
            await DataStore.database.insert('schedule', newSched.toMap());
      },
      onDelete: (deletedSched) async {
        await DataStore.database
            .delete('schedule', where: 'id = ?', whereArgs: [deletedSched.id!]);
      },
      onPressEdit: (sched) async {
        sched.components =
            await DataStore.findComponentsByScheduleId(sched.id!);
...
class _TodoListDisplayState extends State<TodoListDisplay> {
  ...
  Widget build(BuildContext ctx) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("${todolist.name} ($_done/$total)"),
          actions: [
            IconButton(
              ...
              onPressed: () => showDialog(
                context: context,
                builder: (context) =>
                    TaskStatsDialog(stats: getStats(todolist)),
              ),
            )
          ],
        ),
