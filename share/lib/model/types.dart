...
class TodoTask {
  ...
  int completedSubtasks = 0, maxPos = -1;
  ...
  void addSubtask(TodoTask subtask) {
	...
	maxPos = max(maxPos, subtask.position);
	if (subtask.isFinished) completedSubtasks++;
  }
  ...
  void removeSubtaskAt(int i) {
	...
	subtasks.removeAt(i);
	if (subtask.position == maxPos) {
		// recompute maxPos
		maxPos = -1;
		for (var s in subtasks) {
			maxPos = max(s.position, maxPos);
		}
	}
	// if finished was null before
	// and after removing this task
	// if all tasks are finished
	if (finished == null && hasSubtasks 
			&& completedSubtasks == subtasks.length) {
		finished = subtasks
				.map((s) => s.finished)
				.reduce((f1,f2) => f1.isBefore);
	}
  }
...
class TodoList {
  TodoList({required this.name, this.id, this.total = 0, this.completed = 0, required this.tasks});
